...
class TicketSchema(BaseModel):
    ...
    severity: str = Field(
        default="medium", description="严重程度：low/medium/high/critical",
    )
    category: str = Field(
        default="repair", description="工单类别：repair/maintenance/inspection/inquiry",
    )
    status: str = Field(
        default="pending",
        description="状态：pending/dispatched/processing/waiting_parts/resolved/closed/cancelled",
    )
...
class TicketState(TypedDict):
    ...
    session_id: str
    ticket_data: dict | None        # 工单原始数据（从诊断/咨询中提取）
    ticket: dict | None             # TicketSchema.model_dump()
